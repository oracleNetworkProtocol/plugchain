...
本文档解释了 Plugchain 主网的 genesis 文件是如何构建的。 它还解释了如何为自己的`plugchain` testnet 创建一个 genesis 文件。
...
```json
"chain_id": "plugchain-1",
```

## 共识参数

接下来，创世文件定义共识参数。 共识参数覆盖与共识层相关的所有参数，`plugchain` 的共识层是 `Tendermint`。 我们来看看这些参数：
...
```bash
// Example: plugchaind add-genesis-account plug1qs8tnw2t8l6amtzvdemnnsq9dzk0ag0z37gh3h 10000000plug

plugchaind add-genesis-account <account-address> <amount><denom>
```
...
"accounts": [
      {
        "address": "plug1qs8tnw2t8l6amtzvdemnnsq9dzk0ag0z37gh3h",
        "coins": [
          {
            "denom": "plug",
            "amount": "10000000"
...
- `validator_current_rewards`: 验证人的当前奖励相关的信息，由`distribution`模块用于各种计算。 如果没有从之前的状态导出，设置为`null`。
...
- `tally_params`
  ...
  - `veto_threshold`: 提议生效所需`NO_WITH_VETO` 票占总投票数的最大百分比.
...
此命令将存储在`~/.plugchain/config/gentx`中的所有`gentxs`添加到genesis文件中。