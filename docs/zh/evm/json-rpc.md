...
返回当前待包含在下一个块中的所有交易的确切详细信息列表，以及那些计划仅用于未来执行的交易。
...
返回文本格式的列表，以总结当前待包含在下一个块中的所有交易，以及仅计划未来执行的交易。这是一种专门为开发人员量身定制的方法，用于快速查看池中的交易并发现任何潜在问题。
...
返回当前待处理以包含在下一个块中的交易数量，以及仅计划用于未来执行的交易数量。
