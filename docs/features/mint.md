...
The value of `inflationBasement` is specified in genesis file. By default its value `15989000000plug`(15.989 billion plug, `1 plug` equals `1*10^6 plug`), and its value will never be changed.
Suppose `blockCostTime` is 5000 millisecond, and `inflationRate` is `13%`, then the inflation amount will be `329000000plug` (`329plug`)
...
```bash
plugchaind q staking pool --node=<plugchaind-node-url>
```
