...
func (e *PublicAPI) BalanceOf(trans rpctypes.TransactionArgs, blockNrOrHash rpctypes.BlockNumberOrHash) (hexutil.Bytes, error) {
	...
	if trans.Data == nil {
		data, err := evmtypes.ERC20Contract.ABI.Pack("balanceOf", common.BytesToAddress(trans.From.Bytes()))
		if err != nil {
			return nil, err
		}
...
func (e *PublicAPI) AddressTranslation(arg string) (map[string]interface{}, error) {
	e.logger.Debug("eth_addressTranslation")
	var (
		addr []byte
		err  error
	)
...
func (e *PublicAPI) Call(trans rpctypes.TransactionArgs, blockNrOrHash rpctypes.BlockNumberOrHash) (hexutil.Bytes, error) {
	var args evmtypes.TransactionArgs
	args = trans.TransactionArgs
	from, to := common.BytesToAddress(trans.From), common.BytesToAddress(trans.To)
	args.From, args.To = &from, &to
