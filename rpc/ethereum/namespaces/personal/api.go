...
func (api *PrivateAccountAPI) ImportRawKey(privkey, password string) (sdk.AccAddress, error) {
...
func (api *PrivateAccountAPI) ListAccounts() ([]sdk.AccAddress, error) {
	...
	for _, info := range list {
		addrs = append(addrs, sdk.AccAddress(info.GetPubKey().Address().Bytes()))
	}
...
func (api *PrivateAccountAPI) LockAccount(address sdk.AccAddress) bool {
...
func (api *PrivateAccountAPI) NewAccount(password string) (sdk.AccAddress, error) {
...
func (api *PrivateAccountAPI) UnlockAccount(_ context.Context, addr sdk.AccAddress, _ string, _ *uint64) (bool, error) {
...
func (api *PrivateAccountAPI) SendTransaction(_ context.Context, trans rpctypes.TransactionArgs, _ string) (common.Hash, error) {
...
func (api *PrivateAccountAPI) Sign(_ context.Context, data hexutil.Bytes, cosmosAddr sdk.AccAddress, _ string) (hexutil.Bytes, error) {
	api.logger.Debug("personal_sign", "data", data, "address", cosmosAddr.String())

	sig, _, err := api.clientCtx.Keyring.SignByAddress(cosmosAddr, accounts.TextHash(data))
	if err != nil {
		api.logger.Error("failed to sign with key", "data", data, "address", cosmosAddr.String(), "error", err.Error())
		return nil, err
	}
...
func (api *PrivateAccountAPI) EcRecover(_ context.Context, data, sig hexutil.Bytes) (sdk.AccAddress, error) {
	api.logger.Debug("personal_ecRecover", "data", data, "sig", sig)

	if len(sig) != crypto.SignatureLength {
		return sdk.AccAddress{}, fmt.Errorf("signature must be %d bytes long", crypto.SignatureLength)
	}

	if sig[crypto.RecoveryIDOffset] != 27 && sig[crypto.RecoveryIDOffset] != 28 {
		return sdk.AccAddress{}, fmt.Errorf("invalid Ethereum signature (V is not 27 or 28)")
	}
	...
	return sdk.AccAddressFromHex(crypto.PubkeyToAddress(*pubkey).Hex())
}
